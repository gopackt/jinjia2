...
func (e *Evaluator) evalVariable(node *nodes.Variable) (*Value, error) {
	...
	for idx, part := range node.Parts {
		...
		// If current is a reflect.ValueOf(jinjia2.Value), then unpack it
...
func (e *Evaluator) evalParams(node *nodes.Call, fn *Value) ([]reflect.Value, error) {
	...
	for idx, arg := range args {
		...
		if fnArg != typeOfValuePtr {
			// Function's argument is not a *jinjia2.Value, then we have to check whether input argument is of the same type as the function's argument
			if !isVariadic {
				if fnArg != reflect.TypeOf(pv.Interface()) && fnArg.Kind() != reflect.Interface {
					msg := "Function input argument %d of '%s' must be of type %s or *jinjia2.Value (not %T)."
				...
				if fnArg != reflect.TypeOf(pv.Interface()) && fnArg.Kind() != reflect.Interface {
					msg := "Function variadic input argument of '%s' must be of type %s or *jinjia2.Value (not %T)."
			...
			// Function's argument is a *jinjia2.Value
